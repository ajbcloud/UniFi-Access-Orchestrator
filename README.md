...
- **The stick's port heals itself.** After the first good connection, the app records the stick's USB identity (serial number and vendor/product id) under `devices.zwave.serial_identity`. If Windows later renumbers the port (COM3 becomes COM5 after a reboot, a re-plug, or a different USB hub) the app re-finds the same stick by that identity, reconnects, updates the saved port, and logs `serial port auto-updated COM3 -> COM5`; the move also shows in the live event feed. You do not need to re-pick the port. The Devices tab shows the honest state (connected, auto-updated, or, if the stick is truly gone, a "stick not found" hint). On Linux, a `/dev/serial/by-id/...` path is already stable and is left untouched. If you deliberately swap to a different stick, just pick its port and Save; the old identity is dropped automatically. If the stick keeps dropping on Windows without moving, turn off USB selective suspend for that port's root hub (Device Manager, the USB Root Hub, Power Management, untick "Allow the computer to turn off this device").
